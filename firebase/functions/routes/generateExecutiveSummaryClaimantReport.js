              ...
              children: [
                ...
                sampleImageBuffer
                  ...
                  : new Table({
                    ...
                    rows: [
                      new TableRow({
                        height: { value: 1920, rule: "atLeast" }, // keeps image space
                        children: [
                          new TableCell({
                            width: { size: 120, type: WidthType.DXA },
                            margins: { top: 0, bottom: 0, left: 0, right: 0 },
                            children: [
                              new Paragraph({ text: "" }), // empty
                            ],
                          }),
                        ],
                      }),
...
async function addReferralQuestionsContent(children, body) {
  ...
  let rpdrRendered = false;
  let ctpRendered = false;

  const isConsistencyWithDiagnosis = (q) =>
    q.toLowerCase().includes("consistency with diagnosis");
...
async function addFunctionalAbilitiesDeterminationContent(children, body) {
  ...
  for (const [category, tests] of Object.entries(categories)) {
    ...
    for (const test of tests) {
      ...
      const testResultsText = (() => {
        ...
        // Cardio: Display cardio-specific values instead of L/R
        if (category === "Cardio") {
  ...
  // *The sit and stand timeframes
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: "*The sit and stand timeframes are calculated throughout the exam with the individual tests and are not a measure of sustained effort.",
          size: 16,
        }),
      ],
      spacing: { before: 200 },
