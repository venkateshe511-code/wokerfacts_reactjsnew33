            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 6px; line-height: 1;">
                ...
                <tbody>
                    ${(() => {
                      ...
                      const spineROMTests = romTests.filter((test) =>
                        isSpineROMTest(test.testId, test.testName)
                      );
                      const extremityROMTests = romTests.filter(
                        (test) => !isSpineROMTest(test.testId, test.testName)
                      );
                      ...
                      const validateROMTest = (test: any): boolean => {
                        ...
                        for (
                          ...
                          const maxPercDiff = Math.max(
                            (Math.abs(trial1 - avgValue) / avgValue) * 100,
                            (Math.abs(trial2 - avgValue) / avgValue) * 100,
                            (Math.abs(trial3 - avgValue) / avgValue) * 100,
                          );
            ...
            testResults:
              ...
              Object.keys(mtmTestData).length > 0
                ? Object.entries(mtmTestData).map(
                    ([testType, testData]: [string, any]) => ({
                      id: testType,
                      name: testData.testName || getTestNameFromId(testType),
